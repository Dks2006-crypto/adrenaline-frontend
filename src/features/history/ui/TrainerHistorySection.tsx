...
import { useHistoryStats, useTrainerHistory } from "../lib/hooks/useHistory";
import { HistoryFilters as FilterType } from "../model/types";
import HistoryTable from "./HistoryTable";
import HistoryFilters from "./HistoryFilters";

export function TrainerHistorySection() {
  const { user } = useAuthStore();
  const [filters, setFilters] = useState<FilterType>({});
  
  const { data: stats, isLoading: statsLoading } = useHistoryStats();
  const { data: history = [], isLoading: historyLoading, refetch } = useTrainerHistory(
    user?.id,
    filters
  );
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-[#121212] border border-white/10 rounded-2xl p-6">
              <div className="text-green-400 text-sm mb-1">Завершено</div>
            ...
            <div className="bg-[#121212] border border-white/10 rounded-2xl p-6">
              <div className="text-yellow-400 text-sm mb-1">Не явился</div>
        ...
        {/* History Table */}
        <HistoryTable entries={history} isTrainer={true} />
      </div>
    </section>
  );
}
