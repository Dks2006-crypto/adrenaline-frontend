...
export default function HistoryCard({ entry, isTrainer = false }: HistoryCardProps) {
  ...
  return (
    <div className="bg-[#121212] border border-white/10 rounded-2xl p-4 sm:p-6 hover:border-[#1E79AD]/50 transition-all duration-300 group">
      {/* Header with type and status */}
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-bold border ${type.color}`}>
          {type.text}
        </span>
        <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${status.color}`}>
          {status.text}
        </span>
      </div>

      {/* Title */}
      <h3 className="text-white font-bold text-base sm:text-lg mb-3 sm:mb-4 line-clamp-2 overflow-hidden">
        {title}
      </h3>
      ...
      <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-white/80">
        ...
        <div className="flex items-center gap-2">
          <svg className="w-3 h-3 sm:w-4 sm:h-4 text-[#1E79AD]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <span className="text-white/60">Дата:</span>
          <span className="text-white font-medium text-xs sm:text-sm">{dateStr}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-medium text-xs sm:text-sm">{timeStr}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-medium text-xs sm:text-sm">{personName}</span>
        </div>
        ...
        {entry.group_class?.service && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-medium line-clamp-1 overflow-hidden text-xs sm:text-sm">
        ...
        {entry.rating && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-medium text-xs sm:text-sm">{entry.rating}/5</span>
          </div>
        )}
        ...
        {entry.feedback && (
          <div className="pt-2 sm:pt-3 border-t border-white/10">
            ...
            <p className="text-white/80 text-xs sm:text-sm line-clamp-2 overflow-hidden">
              {entry.feedback}
            </p>
          </div>
        )}
